...
class RepaymentCalculator(View):
    ...
    Calculates loan repayment (monthly payment, schedule); returns Chart.js-ready chart_data (BMI-style).
    """
    template_name = 'financial_calculators/repayment_calculator.html'

    def get(self, request):
        context = {'calculator_name': str(_('Repayment Calculator'))}
    ...
    def post(self, request):
        try:
            ...
            loan_amount = self._get_float(data, 'loan_amount_', 0)
            interest_rate = self._get_float(data, 'interest_rate_', 0)
            repayment_period_years = self._get_float(data, 'repayment_period_years', 0)
            loan_term = int(repayment_period_years) if repayment_period_years >= 1 else int(round(repayment_period_years, 0))
            ...
            if loan_term <= 0:
                return JsonResponse({'success': False, 'error': str(_('Repayment period must be at least 1 year.'))}, status=400)
            ...
            result['chart_data'] = self._prepare_chart_data(loan_amount, total_interest, schedule)
    ...
    def _prepare_chart_data(self, principal, total_interest, schedule=None):
        ...
        if schedule:
            ...
            out['schedule_chart'] = schedule_chart
        return out
